...
To run the project, it's sufficient to open it using an IDE like Visual Studio or Rider, and execute the project. The user shall adjust the topic name according to the created one in Elasticsearch, in ``` MainWindow.xaml.cs ```. 